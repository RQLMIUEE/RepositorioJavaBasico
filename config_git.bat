...
git config --global user.name "RQLMIUEE"
git config --global user.email "riquelme.ct.santos@gmail.com"

:: Adiciona o repositório remoto (só executa se já não estiver adicionado)
git remote get-url origin >nul 2>&1
if errorlevel 1 (
    git remote add origin "https://github.com/RQLMIUEE/RepositorioJavaBasico.git"
)
